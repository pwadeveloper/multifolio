...
export const PRICES = Object.freeze({NGN:45000000});
...
export async function verify(reference, env = process.env, fetcher = fetch) {
  ...
  const known = Object.hasOwn(PRICES,data.currency);
  const checks = {
    reference: data.reference === reference,
    package_id: metadata?.package_id === PACKAGE_ID,
    currency: known,
    // Channels where the customer bears the fee report the gross, so require at
    // least the package price. Underpayment is the risk; overpayment is not.
    amount: known && Number(data.amount) >= PRICES[data.currency]
  };
  const failed = Object.keys(checks).filter(name => !checks[name]);
  if (failed.length) {
    logPayment('mismatch',{reference,failed,package_id:metadata?.package_id ?? null,amount:data.amount ?? null,currency:data.currency ?? null,expected:PRICES[data.currency] ?? null});
