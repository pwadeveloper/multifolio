...
for(const name of ['index.html','.rsc','design.rsc','pricing.rsc','works.json','tick.wav','favicon.ico','favicon.svg','favicon-48x48.png','icon-192.png','icon-512.png','apple-touch-icon.png','site.webmanifest']){
 try{await cp(path.join('mirror',name),path.join(out,name));}catch(error){if(error.code!=='ENOENT')throw error;}
}
for(const name of ['design','pricing','checkout','book']){await mkdir(path.join(out,name),{recursive:true});await cp(path.join('mirror',name,'index.html'),path.join(out,name,'index.html'));}
...
for(const name of ['design/index.html','pricing/index.html','design.rsc','pricing.rsc','.rsc']){
