...
for(const name of ['index.html','.rsc','work.rsc','pricing.rsc','works.json','tick.wav','favicon.ico','favicon.svg','favicon-48x48.png','icon-192.png','icon-512.png','apple-touch-icon.png','site.webmanifest']){
 try{await cp(path.join('mirror',name),path.join(out,name));}catch(error){if(error.code!=='ENOENT')throw error;}
}
for(const name of ['work','pricing','checkout','book']){await mkdir(path.join(out,name),{recursive:true});await cp(path.join('mirror',name,'index.html'),path.join(out,name,'index.html'));}
...
for(const name of ['work/index.html','pricing/index.html','work.rsc','pricing.rsc','.rsc']){
