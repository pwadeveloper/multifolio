...
function updatePrice(){const amount='₦2,000 NGN';$('price').textContent=amount;$('billing').textContent=config.billing==='monthly'?'Billed monthly':'One-time payment';$('consent-text').textContent=config.billing==='monthly'?`I authorize ${amount} now and every month for this five-video package until I cancel the subscription through Paystack.`:`I confirm this five-video package and the one-time payment of ${amount}.`;}
async function verify(){
 ...
 if(data.paid){$('heading').textContent='Payment confirmed.';$('intro').textContent='Thank you. Your payment for the content package has been verified.';status('Keep your payment reference and share it with Multimudia when arranging your project brief.');$('price').textContent='₦2,000 NGN';$('billing').textContent=data.billing==='monthly'?'Monthly retainer':'One-time payment';}
