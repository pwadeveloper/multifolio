...
function updatePrice(){const amount='₦450,000 NGN';$('price').textContent=amount;$('billing').textContent=config.billing==='monthly'?'Billed monthly':'One-time payment';$('consent-text').textContent=config.billing==='monthly'?`I authorize ${amount} now and every month for this five-video package until I cancel the subscription through Paystack.`:`I confirm this five-video package and the one-time payment of ${amount}.`;}
async function verify(){
 ...
 if(data.paid){$('heading').textContent='Payment confirmed.';$('intro').textContent='Thank you. Your payment for the content package has been verified.';status('Keep your payment reference and share it with Multimudia when arranging your project brief.');$('price').textContent='₦450,000 NGN';$('billing').textContent=data.billing==='monthly'?'Monthly retainer':'One-time payment';}
 else{const reason=data.gatewayResponse?' Paystack reported: '+data.gatewayResponse:'';status('Your payment has not been confirmed yet.'+reason+' If you completed checkout, check again before making another payment.',true);$('retry').hidden=false;}
 }catch(error){status(error.message,true);$('retry').hidden=false;}
}
$('retry').onclick=verify;
$('checkout-form').onsubmit=async event=>{event.preventDefault();$('pay').disabled=true;status('Opening secure checkout…');try{const result=await request('/api/checkout',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:$('name').value,email:$('email').value,accepted:$('accepted').checked})});location.assign(result.url);}catch(error){status(error.message,true);$('pay').disabled=false;}};
(async()=>{if(reference){await verify();return;}try{config=await request('/api/payment-config');if(!config.enabled){status('Online payment is not open yet. Please book an intro call to discuss your project.');return;}$('checkout-form').hidden=false;$('status').hidden=true;updatePrice();}catch(error){status(error.message,true);}})();
